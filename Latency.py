...
data = pd.read_csv('Final Model/10.193.123.18/Latency/Latency.csv')
data.head()
df = data[['Time','Current Value']]

#Check for NA values
df['Current Value'].isnull().sum()   #0
...
p,d,q = 4,0,1  #p:AR, q:MA d(integ): No. of times to difference the data
...
model = pf.ARIMA(data=dts,ar=4,ma=4,integ=0,target='Current_value') #family is pf.Normal()[By default]
x = model.fit("MLE")
x.summary()
...
model.plot_predict(h=30,figsize=(15,5)) # plots predictions for next 5 time steps, 95% confidence interval
